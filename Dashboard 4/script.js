        ...
        class SmartHomeDashboard {
            constructor() {
                ...
                this.lightModePresets = {
                    normal: { level: 5, color: '#ffffff' }, // 50%
                    warm: { level: 4, color: '#ffd8a6' },   // 40%
                    cool: { level: 6, color: '#a6d8ff' },   // 60%
                    reading: { level: 8, color: '#f5f5dc' }, // 80%
                    party: { level: 10, color: '#ff69b4' }  // 100%
                };
            }
            ...
            switchTab(view) {
                ...
                this.panelTitle.textContent = this.getViewTitle(view);
                this.panelContent.innerHTML = this.getViewContent(view);
    ...
    `;
        ...
        `;
            ...
            `;
                ...
                <div class="control-item">
                    ...
                    <input type="range" min="1" max="10" value="${device.level}" 
                           class="level-slider" 
                           oninput="dashboard.adjustLevel('${roomId}', '${deviceKey}', this.value)">
                    <span style="font-size: 11px; min-width: 30px; text-align: right;">${device.level}/10</span>
                </div>
        ...
        `;
            ...
            `;
                        ...
                        if (device.status) {
                            content += `
                <div class="control-item">
                    <div class="control-label">Speed (1-10)</div>
                    <input type="range" min="1" max="10" value="${device.level}" 
                           class="level-slider" 
                           oninput="dashboard.adjustLevel('${roomId}', '${deviceKey}', this.value)">
                    <span style="font-size: 11px; min-width: 30px; text-align: right;">${device.level}/10</span>
            ...
            adjustLevel(roomId, deviceKey, value) {
                ...
                if (span) span.textContent = `${device.level}/10`;
                ...
                this.logActivity(`${device.name} level set to ${device.level}/10`);
            }
            ...
            toggleDevice(roomId, deviceKey) {
                ...
                if (device.status && !wasOn) {
                    if (device.type === 'light') {
                        ...
                        device.level = device.level || preset.level || 5;
                        device.color = device.color || preset.color || '#ffffff';
                        device.mode = device.mode || 'normal';
                    }
                    if (device.type === 'fan') {
                        // For fans, use previous level or default to 5
        ...
        roomAction(roomId, action) {
            ...
            switch (action) {
                case 'allOn':
                    Object.values(room.devices).forEach(device => device.status = true);
                    break;
                case 'allOff':
                    Object.values(room.devices).forEach(device => device.status = false);
                    break;
                case 'nightMode':
        ...
        quickAction(action) {
            switch (action) {
                case 'allLightsOn':
                    Object.keys(this.rooms).forEach(roomId => {
                        Object.entries(this.rooms[roomId].devices).forEach(([key, device]) => {
                            if (device.type === 'light') device.status = true;
                        });
                        this.updateRoomState(roomId);
                    });
                    break;
                case 'allLightsOff':
                    Object.keys(this.rooms).forEach(roomId => {
                        Object.entries(this.rooms[roomId].devices).forEach(([key, device]) => {
                            if (device.type === 'light') device.status = false;
                        });
                        this.updateRoomState(roomId);
                    });
                    break;
    ...
    `;
        ...
        showHistory() {
            ...
            const historyContent = this.history.slice(0, 10).map(entry => `
